...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  label_id = label_map.get(example.label.encode('utf-8'),0)
  if ex_index < 5:
...
def main(_):
  ...
  if FLAGS.do_predict:
    ...
    model_predict = predictor.from_saved_model('./output/reason_output')

    while True:
        s=raw_input('Type in a source sequence:')
        text_li = list(jieba.cut(s))
        seq = []
        for word in text_li:
            if word not in stopwords:
                if word in ['未','不'] or len(word)>1:
                    seq.append(word)
        s=''.join(seq)

        predict_examples = processor.get_single_example(s)
